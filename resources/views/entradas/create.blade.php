        ...
        <form method="POST" action="{{ route('entradas.store') }}" id="form-entrada" class="space-y-6" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="responsavel_recebimento_id" id="responsavel_recebimento_id" class="block w-full rounded-lg border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-gray-900 sm:text-sm">
                        <option value="">Selecione o responsável</option>
                        @foreach ($responsaveisRecebimento as $responsavel)
                            <option value="{{ $responsavel->id }}" data-cd="{{ $responsavel->cd_id }}" @selected(old('responsavel_recebimento_id') == $responsavel->id)>{{ $responsavel->nome }}</option>
                        @endforeach
                    </select>
        ...
        (function () {
            ...
            @if ($podeEscolherCd)
                ...
                cdSelect.addEventListener('change', function () {
                    ...
                    filtrarPorCd('responsavel_recebimento_id');
                });
