        ...
        <form method="POST" action="{{ route('saidas.store') }}" id="form-saida" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-select-input id="motivo_saida_id" name="motivo_saida_id" :options="$motivos" placeholder="Selecione o motivo" />
                ...
                <div>
                    ...
                    <select name="colaborador_id" id="colaborador_id" class="block w-full rounded-lg border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-gray-900 sm:text-sm">
                        ...
                        @foreach ($colaboradores as $colaborador)
                            <option value="{{ $colaborador->id }}" data-cd="{{ $colaborador->cd_id }}" @selected(old('colaborador_id') == $colaborador->id)>{{ $colaborador->nome }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="liberado_por" id="liberado_por" class="block w-full rounded-lg border-0 py-1.5 px-3 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 focus:ring-2 focus:ring-gray-900 sm:text-sm">
                        ...
                        @foreach ($usuarios as $usuario)
                            <option value="{{ $usuario->id }}" data-cd="{{ $usuario->cd_id }}" @selected(old('liberado_por', auth()->id()) == $usuario->id)>{{ $usuario->name }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <x-text-input type="date" id="data" name="data" :value="old('data', now()->toDateString())" />
                    <x-input-error name="data" />
                </div>

                <div>
                    <x-input-label for="hora" required>Hora</x-input-label>
                    <x-text-input type="time" id="hora" name="hora" :value="old('hora', now()->format('H:i'))" />
        ...
        (function () {
            ...
            @if ($podeEscolherCd)
                ...
                cdSelect.addEventListener('change', function () {
                    filtrarPorCd('colaborador_id');
                    filtrarPorCd('liberado_por');
                });
